...
const fallbackCities = [
  ...
  { id: "3", name: "Karnataka", state: "Karnataka", college_count: 420, image_url: "https://images.unsplash.com/photo-1596176530529-78163a4f7af2?w=400&h=300&fit=crop" },
  { id: "4", name: "Tamil Nadu", state: "Tamil Nadu", college_count: 350, image_url: "https://images.unsplash.com/photo-1582510003544-4d00b7f74220?w=400&h=300&fit=crop" },
  { id: "5", name: "Haryana", state: "Haryana", college_count: 310, image_url: "https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=400&h=300&fit=crop" },
    ...
    <section className="py-10 md:py-14" aria-labelledby="city-search-heading">
      <div className="container">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-8">
          ...
          <p className="mt-2 text-muted-foreground text-sm">Find the best colleges across India's top education hubs</p>
        </motion.div>

        {/* Image card grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
          {cities.slice(0, 8).map((place, i) => (
            <motion.div
              key={place.id}
            ...
            >
              <Link
                ...
                className="group relative block h-40 sm:h-48 md:h-52 rounded-2xl overflow-hidden"
              >
                <img
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <h3 className="text-lg md:text-xl font-bold text-card uppercase tracking-wide">
                    {place.name}
                  </h3>
                  <span className="text-xs text-card/80">{place.college_count}+ Colleges</span>
                </div>
              </Link>
            </motion.div>
          ))}
        ...
        <div className="text-center mt-6">
          <Link to="/colleges">
            <Button variant="outline" size="lg" className="rounded-xl border-border hover:bg-primary/5">
