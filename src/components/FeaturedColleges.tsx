        ...
        >
          <div>
            ...
            <h2 id="featured-heading" className="text-headline font-bold text-foreground">
              Top Ranked <span className="text-primary">Colleges</span>
            </h2>
            ...
            {topColleges.map((college, index) => (
              ...
              >
                ...
                <div className="p-4 space-y-2.5">
                  <div>
                    <h3 className="text-base font-bold text-foreground">{college.short_name}</h3>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={`w-3.5 h-3.5 ${i < Math.floor(college.rating) ? "fill-primary text-primary" : "text-muted-foreground/30"}`} />
                      ))}
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <Link to={`/colleges/${college.slug}`}>
                      <Button variant="outline" size="sm" className="w-full rounded-lg text-xs h-9">Know More</Button>
                    </Link>
                    <Button size="sm" className="w-full rounded-lg text-xs h-9 bg-primary text-primary-foreground hover:bg-primary/90">Apply Now</Button>
                  </div>
                </div>
              </motion.article>
            ))}
